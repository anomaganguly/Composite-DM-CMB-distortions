# Python code to :
# find best fit epsilon for global absorption signal from dark matter in the
#COBE band using MCMC for set of masses and energy splittings
...
mass_expo = 1.E-1 #MeV# #m_f# 
delE = 1.35E1  #eV #s_d# 0.1348685591967376 28.14333391950896
...
# path1 = '/user1/anoma/cobe_zoo_dynesty/COBE_data.txt'
...
T_0 = 2.725
xf = hp*freq*1.E9/(kb*T_0)
# Transition temperature range for DM absorption in 50-10^5 redshift range
T_starmax = np.min(freq)*(1.E5)*1.E9*hp/kb
T_starmin = np.max(freq)*(1+50)*1.E9*hp/kb
...
def Edmcmb(z, T, T_star, mdm, alpha_A, a1):
    ...
    if T[1]<=0:
        ...
        factor = -kb*T_star*ndm(z, mdm)*(n10(T[0], T_star)/(1+n10(T[0], T_star)))*C10(z, mdm, a1, 0.5, T[1])
        return factor*(1 - np.exp(-T_star*(1/T[1]-1/T[0])))/((3./2.)*ndm(z, mdm)*H_z(z)*kb*T[1])
    # elif ndm(z, mdm)*a1*mdm*c*np.power((8*kbev*T[1]*1.e-6/(np.pi*mevdm)),0.5)/H_z(z)>=1 and T[1]>=0:
    #     return edot(z, T[0], T_star, mdm, alpha_A)/((3./2.)*ndm(z, mdm)*H_z(z)*kb*T[1])
    # else:
    #      return 0.
...
def edot(z, T, T_star, mdm, alpha_A):
    ...
    if xcmb(z, T_star)>100:
        echange = factor*((g10 - n10(T, T_star))*(np.exp(-xcmb(z, T_star)))-n10(T, T_star))    
    else:
        echange = factor*((g10 - n10(T, T_star))/(np.exp(xcmb(z, T_star))-1)-n10(T, T_star))    
    return echange #factor*((g10 - n10(T, T_star))/(np.exp(xcmb(z, T_star))-1)-n10(T, T_star))
...
# path2 = '/user1/anoma/cobe_zoo_dynesty/rec.txt'
...
# path3 = '/user1/anoma/cobe_zoo_dynesty/gaunt_factors.txt'
...
def u_dm(mdm, T_star, z_lmin, z_lmax):
    ...
    if z_lmin<z_c:
        mupar = 1.4*(-edm(mdm, 3.)/epl(3.))*\
            (np.log((1+zmax)/(1+10**z_c))-(4./3.)*(aR/(bR*kb))*(2.726)/(T_star)*(zmax-10**z_c))
        ypar = 0.25*(-edm(mdm, 3.)/epl(3.))*(np.log((1+10**z_c)/(1+zmin)))
    else:
        mupar = 1.4*(-edm(mdm, 3.)/epl(3.))*\
            (np.log((1+zmax)/(1+zmin))-(4./3.)*(aR/(bR*kb))*(2.726)/(T_star)*(zmax-zmin))
        ypar = 0.
    return (mupar, ypar) 

# Solving for Tex+Tdm
def Tex_solver(T_star, mdm, alpha_A, a1, beta, teval, Tdm, Tex):
    solver = ode('cvode', lambda z, T, Tdot: f_Tex(z, T, Tdot, T_star, mdm, alpha_A, a1, beta), old_api=False, rtol=1.e-10, max_steps=1.e8)
    sol = solver.solve(teval, [Tex, Tdm])
    return sol

# Solving for Tdm only
def Tdm_solver(T_star, mdm, alpha_A, a1, teval, Tdm):
    solver = ode('cvode', lambda z, T, Tdot: f_Tdm(z, T, Tdot, T_star, mdm, alpha_A, a1), old_api=False, rtol=1.e-10, max_steps=1.e8)
...
def dm_signal(f, T_star, mdm, eps, a1, beta):
    ...
    if eps == 0. or a1==0:
        ...
        return (np.zeros(f.shape), np.zeros(f.shape)) #no signal
    elif zdec==0.:
        print('DM coupled after z_eq')
        return (np.ones(f.shape), np.ones(f.shape)) #not allowed!
    else:
        # Range of z for T_ex evaluation 
        # zmin = 50.
        zmin = np.log10(invert1(np.max(f), T_star))
        zmax = np.log10(zdec)
        ...
        if ah>1 and ch>1:   # collisions and radiative transitions above hubble
            if ca>1:        # collisions > radiative transitions
                if ca>1.e4 and ch>1.e4: # collisions >> radiative transitions and collisions >> Huuble
                    while(zmax-1>=np.log10(zmin)):
                        teval = np.linspace(np.log10(10**zmax), np.log10(10**(zmax-0.01)), num=10)
                        ...
                        if ca<1.e7 and ch<1.e7: # C and A start becoming comparable
                            break
                ...
                if ah>1.e4 and ca<1.e-4: # A>1000C and A>1000H: radiation dominates fully and mu+y distortion contributes to u 
                    if zmax > z_bb:
                        zumax = z_bb
                    else:
                        zumax = zmax
            ...
            if ch>1.e4 and ca>1.e4: # collisions >> Hubble and collisions >> radiative transitions
                while(zmax-1>=np.log10(zmin)):
                    ...
                    zmax = z_arr[-1][-1]
                    ch, ah, ca = Rcomp(zmax, mdm, a1, A10dm, beta, Tdm, T_star, Tex)
                    if ch<1.e7 and ca<1.e7: # C and A start becoming comparable
                        break
            # collisions > Hubble and rad transitions < Hubble ... comparable rates
            zc = zmax
            zc0 = zmax
            while(zc-1>=np.log10(zmin)):
                teval = np.linspace(np.log10(10**zc), np.log10(10**(zc-0.01)), num=10) #evaluation points in redshift for T_ex
                solution = Tex_solver(T_star, mdm, alpha_A, a1, beta, teval, Tdm, Tex)
                z_arr.append(solution.values.t)
                Tex_arr.append(solution.values.y[:,0])
                Tdm_arr.append(solution.values.y[:,1])
        ...
        if len(z_arr)==0:
            # print(ah, ch, ca)
            # print('problem!')
            return (np.zeros(f.shape), np.zeros(f.shape))
        else:
...
############### mu + y signal ##########################
        muplusysignal = (mu_dm[0]+mu_i)*(-T_0/xf)*dBdT(f) + (y_dm[0]+y_i)*T_0*(xf*(np.exp(xf)+1)/(np.exp(xf)-1)-4)*dBdT(f)

############## global signal ###########################
        if T_starmin<=T_star<=T_starmax:
            zgs = np.log10(invert1(f, T_star))  # redshift array of absorption
            T_exgs = np.zeros(len(zgs))         # initializing Tex
            global_signal = np.zeros(len(zgs))  # initializing change in intensity due to global feature

            for i in range(len(zgs)):
                if zgs[i]>z_c or zgs[i]<np.log10(50) or zgs[i]>flat_z[0] or zgs[i]<flat_z[-1]: # when TDM/Tex coupled to TCMB
                    T_exgs[i] = Tcmb(zgs[i])
                    global_signal[i] = 0.
                else:
                    T_exgs[i] = T_exint(zgs[i])
                    global_signal[i] = (2*kb*(f[i]*1.E-1/(c*1.E-10))**2)*Tdb(zgs[i], T_exgs[i], T_star, mdm, alpha_A)*1.E20
        else:
            global_signal = np.zeros(f.shape)
        # shift_signal = 5.e-6*dBdT(f)
        return  (muplusysignal, global_signal)    
...
def model_signal(f, T_star, mdm, eps, a1, beta, delT, G0):
    ...
    if dmsignal[0][0]==1: #case where dm is coupled to sm till matter-radiation eq
        return -np.inf
    else:
        return dmsignal[0] + dmsignal[1] + (delT*dBdT(f) +  G0*galnu(f)) 

def log_prior(theta):  #log of prior if prob = 1 (log prob = 0) and prob = 0 (log prob = -inf)
    Eps, DelT, g0 = theta
    if 0. <= Eps <= 1.E-3 and -1.E-2 <= DelT <= 1.E-2 and -1.0 <= g0 <=1.0:
        return 0.0
    return -np.inf
...
def get_params(theta):
    Eps, DelT, g0 = theta # parameters
    a1 = coll_par
    return Eps, a1, 0.5, DelT, g0

def log_likelihood(theta, x, y, yerr):
    eps, a1, beta, DelT, g0 = get_params(theta)
    model = model_signal(x, delE/kbev, (10**mass_expo)*Mev, eps, a1, beta, DelT, g0) ### change del E and mass
    sigma2 = yerr**2 
    if not np.isfinite(np.sum(model)):
        return -np.inf
    else:
...
pos0 = np.asarray([1.E-7, -0.01, -1.0])
pos1 = np.asarray([1.E-3, 0.01, 1.0])
pos =  pos0 + np.random.random_sample((100, 3))*(pos1-pos0)  #100 walkers 3 params
# print(pos)
...
filename = '/home/anoma/final_codes/gs/emcee_sample_M_'+num0+'_delE_'+num1+'.h5'
# filename = "/home/anoma/cobe_zoo_mcmc/sample1.h5"
...
sampler = emcee.EnsembleSampler(nwalkers, ndim, log_probability, args=(freq, residual, del_intensity), backend = backend, threads=20)
...
# plt.show()
plt.savefig('/home/anoma/final_codes/gs/emcee_chain_M_'+num0+'_delE_'+num1+'.pdf')
...
plt.savefig('/home/anoma/final_codes/gs/emcee_corner_M_'+num0+'_delE_'+num1+'.pdf')
